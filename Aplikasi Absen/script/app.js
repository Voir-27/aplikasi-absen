...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (deleteKelasBtn) {
      deleteKelasBtn.addEventListener('click', async function () {
        ...
        if (!currentClassEntry.custom) {
          alert('Hanya kelas custom yang dapat dihapus.');
          return;
        }
        ...
        // hapus entry dari custom classes
        try {
        ...
        // NEW: jika ada directory handle, coba hapus file fisik relatif
        if (__dirHandle && currentClassEntry && currentClassEntry.file) {
          try {
            const removed = await removeFileInDir(__dirHandle, currentClassEntry.file);
            if (removed) {
              alert('File JSON untuk kelas juga dihapus dari folder yang Anda pilih.');
            } else {
              // tidak ditemukan atau gagal
            }
          } catch (err) {
            console.warn('Gagal menghapus file fisik via directory handle', err);
          }
        }
...
function renderCustomClasses() {
  ...
  if (!menu) return;
  // hapus entri custom lama (beri attribute data-custom="1" saat buat)
...
async function createAndSaveJson(suggestedFilename, content) {
  // jika user sudah memilih directory via permission, tulis langsung ke sana
  if (__dirHandle) {
    try {
      await writeFileInDir(__dirHandle, suggestedFilename, content);
      alert(`File disimpan ke folder yang Anda pilih: ${suggestedFilename}`);
      return;
    } catch (err) {
      console.warn('Gagal menulis file ke directory handle, fallback ke picker/download:', err);
      // lanjut ke fallback ke showSaveFilePicker / download
    }
  }
...
// NEW: global directory handle (disimpan hanya di runtime)
...
// NEW: request filesystem permission and store handle
async function requestFSPermission() {
  try {
    if (!window.showDirectoryPicker) {
      alert('File System Access API tidak tersedia pada browser ini. Akan menggunakan fallback (download file).');
      return;
    }
    // minta user pilih folder
    const dir = await window.showDirectoryPicker();
    // minta permission read/write
    const perm = await dir.requestPermission({ mode: 'readwrite' });
    if (perm === 'granted' || perm === 'prompt') {
      __dirHandle = dir;
      alert('Izin diberikan. Aplikasi sekarang dapat membuat / menghapus file di folder yang dipilih selama sesi ini.');
    } else {
      alert('Izin tidak diberikan.');
    }
  } catch (err) {
    console.warn('Permintaan izin filesystem dibatalkan/terjadi error', err);
    alert('Gagal meminta izin filesystem: ' + (err && err.message ? err.message : err));
  }
}

// NEW helpers: tulis file relatif ke directory handle (membuat subfolder bila perlu)
async function writeFileInDir(dirHandle, relativePath, content) {
...
// NEW helper: hapus file relatif di directory handle (jika ada)
async function removeFileInDir(dirHandle, relativePath) {
  const parts = relativePath.split('/').filter(Boolean);
  if (parts.length === 0) throw new Error('Invalid path');
  let dir = dirHandle;
  for (let i = 0; i < parts.length - 1; i++) {
    dir = await dir.getDirectoryHandle(parts[i], { create: false }).catch(() => null);
    if (!dir) return false; // folder tidak ada
  }
  const fileName = parts[parts.length - 1];
  try {
